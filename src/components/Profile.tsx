...
const Profile = () => {
  ...
  const [showDeleteAccountModal, setShowDeleteAccountModal] = useState<boolean>(false);   // controls delete account modal
...
const handleDeleteAccount = async (): Promise<void> => { // function for deleting account

  try {

    const storedUser = JSON.parse(localStorage.getItem("user") || "{}"); // get logged in user

    await fetch(`https://69ef23b6112e1b968e240e58.mockapi.io/users/${storedUser.id}`, { // inserts current user's id into URL
      method: "DELETE"
    });

    localStorage.removeItem("user"); // remove logged in user

    localStorage.removeItem(`tasks_${storedUser.email}`); // remove user tasks

    navigate("/"); // redirect to login page

  } catch (error) {

    console.log("Delete failed:", error); // show error if request fails
  }
};
  ...
  <>
    ...
    {showConfirmModal && (
      <div className="absolute inset-0 z-99999 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
        <div className={`rounded-4xl p-6 md:p-8 max-w-sm w-full shadow-2xl border ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-purple-50"}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
        ...
        <div className={cardStyle}>
          ...
          <div className="space-y-4 mt-6">
            ...
            <div>
              <label className="text-[10px] font-black text-[#94A3B8] uppercase block mb-1 ml-1">
                Password
              </label>
              <div className={`flex items-center justify-between p-4 rounded-2xl border ${isDark ? "bg-slate-800/50 border-slate-700" : "bg-[#FAF9FF] border-purple-50"}`}>
                ...
                <button
                  ...
                  className="text-[#94A3B8] hover:text-[#6D28D9] transition-colors p-1"
                >
        ...
        <div className={`${cardStyle} md:col-span-2 lg:col-span-1`}>
          ...
          <div className={`pt-4 border-t space-y-3 mt-6 ${isDark ? "border-slate-800" : "border-purple-100"}`}>
            <p className="text-[11px] text-[#94A3B8] leading-relaxed font-medium">
              Permanently remove your account and all associated tasks.
